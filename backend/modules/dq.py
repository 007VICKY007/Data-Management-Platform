...
# ═══════════════════════════════════════════════════════════════════════
#  PERF: Shared null sentinel set & prep helper (avoid re-creating)
# ═══════════════════════════════════════════════════════════════════════
_NULL_SENTINELS = frozenset({"", "nan", "none", "null", "na", "n/a", "n.a.", "nil", "missing", "#n/a"})

def _prep_series(series: pd.Series) -> pd.Series:
    """PERF: Single str prep call — reused across rules on same column."""
...
def rule_not_empty(series: pd.Series) -> pd.Series:
    ...
    return _prep_series(series).str.len() > 0
...
def rule_pan_format(series: pd.Series) -> pd.Series:
    ...
    s = _prep_series(series).str.upper()
    skip = _is_empty_for_validity(s)
...
def rule_allowed_values(series: pd.Series, allowed: List[str]) -> pd.Series:
    s = _prep_series(series)
    skip = _is_empty_for_validity(s)
    allowed_lower = frozenset(v.strip().lower() for v in allowed)
...
def detect_exact_duplicates_combination(
    ...
    """PERF: Vectorized combo duplicate detection — no axis=1 apply."""
    combo = df[cols].astype(str).apply(lambda x: x.str.strip().str.lower())

    # PERF: Vectorized all-null check using NumPy
    null_matrix = np.isin(combo.values, list(_NULL_SENTINELS))
    all_null_mask = pd.Series(null_matrix.all(axis=1), index=df.index)
    ...
    # PERF: Vectorized key construction using column concatenation
    combo_key = combo_valid.iloc[:, 0]
    for i in range(1, len(cols)):
        combo_key = combo_key + "||" + combo_valid.iloc[:, i]
    ...
    for _, block_group in work[work["_block"] != ""].groupby("_block", sort=False):
        ...
        if RAPIDFUZZ_AVAILABLE and n_pairs <= max_pairs_per_block:
            ...
            if score_matrix is not None:
                # PERF: Use NumPy triu_indices for upper triangle
                i_arr, j_arr = np.triu_indices(n, k=1)
                scores_arr = score_matrix[i_arr, j_arr]
                above = scores_arr >= threshold
                if above.any():
                    idx_array = np.array(idxs)
                    for ii, jj, sc in zip(i_arr[above], j_arr[above], scores_arr[above]):
                        matches.append({
                            "_i": idx_array[ii],
                            "_j": idx_array[jj],
                            "_score": float(sc),
...
def build_clean_dataset(
    ...
    # PERF: Vectorized Issue_Flag using numpy
    issue_rows = set(int(r["Row_Number"]) for r in all_annexure)
    base = base.copy()
    base["Issue_Flag"] = np.where(np.isin(base.index + 2, list(issue_rows)), "Yes", "No")
...
# ═══════════════════════════════════════════════════════════════════════
#  PERF: Pre-cached openpyxl style objects — created once, reused everywhere
# ═══════════════════════════════════════════════════════════════════════
_STYLE_CACHE = {}

def _get_cached_styles():
    """Create and cache all reusable openpyxl style objects."""
    if _STYLE_CACHE:
        return _STYLE_CACHE
    ...
    _STYLE_CACHE.update({
        "border": border,
        "header_font": Font(bold=True, color=_CLR_WHITE, size=11, name="Arial"),
        "header_fill": PatternFill(start_color=_CLR_HEADER_DARK, end_color=_CLR_HEADER_DARK, fill_type="solid"),
        "header_align": Alignment(horizontal="center", vertical="center", wrap_text=True),
        "data_font": Font(size=10, name="Arial"),
        "data_align": Alignment(vertical="center", wrap_text=True),
        "fill_white": PatternFill(start_color=_CLR_WHITE, end_color=_CLR_WHITE, fill_type="solid"),
        "fill_alt": PatternFill(start_color=_CLR_ALT_ROW, end_color=_CLR_ALT_ROW, fill_type="solid"),
        "fill_good": PatternFill(start_color=_CLR_GOOD, end_color=_CLR_GOOD, fill_type="solid"),
        "fill_moderate": PatternFill(start_color=_CLR_MODERATE, end_color=_CLR_MODERATE, fill_type="solid"),
        "fill_poor": PatternFill(start_color=_CLR_POOR, end_color=_CLR_POOR, fill_type="solid"),
        "font_good": Font(bold=True, color=_CLR_GOOD_FONT, size=10, name="Arial"),
        "font_moderate": Font(bold=True, color=_CLR_MODERATE_FONT, size=10, name="Arial"),
        "font_poor": Font(bold=True, color=_CLR_POOR_FONT, size=10, name="Arial"),
        "center_align": Alignment(horizontal="center", vertical="center"),
        "annex_header_fill": PatternFill(start_color="1F3864", end_color="1F3864", fill_type="solid"),
        "annex_header_font": Font(bold=True, color="FFFFFF", size=10, name="Arial"),
        "annex_fill_alt": PatternFill(start_color="EBF3FB", end_color="EBF3FB", fill_type="solid"),
        "annex_fill_white": PatternFill(start_color="FFFFFF", end_color="FFFFFF", fill_type="solid"),
    })
    return _STYLE_CACHE


def _apply_header_style(ws, row_num: int, fill_color: str = _CLR_HEADER_DARK) -> None:
    """PERF: Uses cached styles."""
    styles = _get_cached_styles()
    from openpyxl.styles import PatternFill
    fill = styles["header_fill"] if fill_color == _CLR_HEADER_DARK else PatternFill(start_color=fill_color, end_color=fill_color, fill_type="solid")
...
def _apply_data_style(ws, start_row: int, end_row: int, num_cols: int, zebra: bool = True) -> None:
    """PERF: Uses cached style objects instead of creating new ones per cell."""
    styles = _get_cached_styles()
    border = styles["border"]
    data_font = styles["data_font"]
    data_align = styles["data_align"]
    fill_alt = styles["fill_alt"]
    fill_white = styles["fill_white"]
    for r in range(start_row, end_row + 1):
        fill = fill_alt if (zebra and r % 2 == 0) else fill_white
...
def _color_score_cell(cell, score) -> None:
    """PERF: Uses cached style objects."""
    if not isinstance(score, (int, float)):
        return
    styles = _get_cached_styles()
    if score > 95:
        cell.fill = styles["fill_good"]
        cell.font = styles["font_good"]
    elif score >= 85:
        cell.fill = styles["fill_moderate"]
        cell.font = styles["font_moderate"]
    else:
        cell.fill = styles["fill_poor"]
        cell.font = styles["font_poor"]
...
def _annexure_header_style(ws, row_num: int) -> None:
    ...
    for cell in ws[row_num]:
        if cell.value is not None:
            cell.font = styles["annex_header_font"]
            cell.fill = styles["annex_header_fill"]
            cell.alignment = styles["header_align"]
            cell.border = styles["border"]
...
def _annexure_data_style(ws, start_row: int, end_row: int, num_cols: int) -> None:
    ...
    data_font = styles["data_font"]
    data_align = styles["data_align"]
    fill_alt = styles["annex_fill_alt"]
    fill_white = styles["annex_fill_white"]
    for r in range(start_row, end_row + 1):
...
def _build_row_issue_map(
    ...
    For each row we collect:
      issues_text    – human-readable semicolon-separated issue labels
      issue_count    – number of distinct issues on that row
      categories     – comma-separated dimension names (sorted)
      affected_cols  – semicolon-separated column names that failed
      rules_applied  – semicolon-separated rule names applied
      original_vals  – semicolon-separated original values that failed
    ...
    for rec in all_annexure:
        ...
        entry = row_map.setdefault(row_idx, {
            "issues": [], "dims": set(),
            "columns": [], "rules": [], "orig_vals": [],
        })
        ...
        # Track affected column, rule, and original value
        col_name = str(rec.get("Column_Name", "")).strip()
        rule_name = str(rec.get("Rule_Applied", "")).strip()
        orig_val = str(rec.get("Original_Value", "")).strip()

        detail = f"{col_name}: {rule_name}" if col_name else rule_name
        if detail and detail not in entry["rules"]:
            entry["rules"].append(detail)
        if col_name and col_name not in entry["columns"]:
            entry["columns"].append(col_name)
        if orig_val and orig_val not in ("", "nan", "None", "NaN"):
            entry["orig_vals"].append(f"{col_name}={orig_val}" if col_name else orig_val)

    result: Dict[int, Dict[str, Any]] = {}
    for idx, entry in row_map.items():
        result[idx] = {
            "issues_text":    "; ".join(entry["issues"]),
            "issue_count":    len(entry["rules"]),  # count of column+rule pairs
            "categories":     ", ".join(sorted(entry["dims"])),
            "affected_cols":  "; ".join(entry["columns"]),
            "rules_applied":  "; ".join(entry["rules"]),
            "original_vals":  "; ".join(entry["orig_vals"][:10]),
        }
    return result
...
def _sheet_results(
    ...
    Columns = all original df columns + Issues | Count of issues | Issue categories
              | Affected Columns | Rules Applied | Original Values.
    """
    headers = list(df.columns) + [
        "Issues", "Count of issues", "Issue categories",
        "Affected Columns", "Rules Applied", "Original Values",
    ]
    for c, h in enumerate(headers, 1):
    ...
    out_row = 2
    # PERF: Extract issue rows via numpy for fast cell writing
    issue_indices = sorted(row_issue_map.keys())
    valid_indices = [i for i in issue_indices if 0 <= i < len(df)]
    if valid_indices:
        subset_vals = df.iloc[valid_indices].values  # numpy array
        nc = len(df.columns)
        for arr_idx, df_idx in enumerate(valid_indices):
            info = row_issue_map[df_idx]
            row_vals = subset_vals[arr_idx]
            for c in range(nc):
                ws.cell(row=out_row, column=c + 1, value=_safe_val(row_vals[c]))
            ...
            ws.cell(row=out_row, column=nc + 4, value=info.get("affected_cols", ""))
            ws.cell(row=out_row, column=nc + 5, value=info.get("rules_applied", ""))
            ws.cell(row=out_row, column=nc + 6, value=info.get("original_vals", ""))
...
def _sheet_rule_annexure(
    ...
    for out_row, idx in enumerate(row_indices, 2):
        # PERF: Use .iloc for positional access with numpy values
        try:
            df_row_vals = df.iloc[idx].values
        except IndexError:
            continue
        for c in range(len(df_row_vals)):
            ws.cell(row=out_row, column=c + 1, value=_safe_val(df_row_vals[c]))
        nc = len(df.columns)
        info = row_issue_map.get(idx, {
            "issues_text": rule_label, "issue_count": 1, "categories": "",
            "affected_cols": "", "rules_applied": rule_label, "original_vals": "",
        })
        ws.cell(row=out_row, column=nc + 1, value=info["issues_text"])
        ws.cell(row=out_row, column=nc + 2, value=info["issue_count"])
        ws.cell(row=out_row, column=nc + 3, value=info["categories"])
        ws.cell(row=out_row, column=nc + 4, value=info.get("affected_cols", ""))
        ws.cell(row=out_row, column=nc + 5, value=info.get("rules_applied", rule_label))
        ws.cell(row=out_row, column=nc + 6, value=info.get("original_vals", ""))
        ws.row_dimensions[out_row].height = 18
...
def _sheet_dup_annexure(
    ...
    for out_row, (_, dr) in enumerate(dup_records_clean.iterrows(), 2):
        dr_vals = dr.values
        for c in range(len(dr_vals)):
            ws.cell(row=out_row, column=c + 1, value=_safe_val(dr_vals[c]))
        ws.row_dimensions[out_row].height = 18
...
def generate_excel_report(
    ...
    # PERF: Initialize style cache early for all sheet writers
    _get_cached_styles()
