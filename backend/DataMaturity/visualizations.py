...
KEY CHANGE — Per-Dimension Objects
====================================
render_slide_png now accepts objects_per_dim parameter.
The domain_scores dict already contains averaged scores per dimension,
so the visualization itself doesn't need to know which objects each
dimension has — the averaging is done in the scoring layer.
"""
...
def _draw_maturity_wheel(ax) -> None:
    ...
    x_bus_positions = [0.268, 0.276, 0.284, 0.292, 0.300]
    x_dot = 0.315
    x_txt = 0.326
    ...
    for title, desc, y_t, idx in callouts:
        ...
        x0  = cx + ro * np.cos(rad)
        y0  = cy + ro * np.sin(rad)
        x_bus = x_bus_positions[idx]

        ax.plot([x0, x_bus], [y0, y0],     transform=ax.transAxes, color=magenta, lw=1.0, zorder=2)
        ax.plot([x_bus, x_bus], [y0, y_t], transform=ax.transAxes, color=magenta, lw=1.0, zorder=2)
        ax.plot([x_bus, x_dot], [y_t, y_t], transform=ax.transAxes, color=magenta, lw=1.0, zorder=2)
        ax.scatter([x_dot], [y_t], transform=ax.transAxes, s=16,
                   color="white", edgecolor=magenta, linewidth=1.1, zorder=4)
...
def _draw_domain_table(ax, domain_scores: dict) -> None:
    """Right-panel table with maturity-scale colored mini bars."""
...
def render_slide_png(
    ...
    domain_scores is {dim_name: float} — already averaged across
    that dimension's objects (which may differ per dimension).
    objects_per_dim is accepted for signature compatibility but
    the slide renders domain_scores as-is.
    """
    _set_global_font()
